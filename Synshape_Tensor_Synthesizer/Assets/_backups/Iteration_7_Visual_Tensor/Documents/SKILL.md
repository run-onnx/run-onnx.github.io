...
| Ambiguous â€œç”»ä¸ªå›¾/åšä¸ªå›¾/åšä¸ªç¤ºæ„/å¸®æˆ‘å¯è§†åŒ–/design a page/åšä¸ªç•Œé¢/åšä¸ªåŽŸåž‹â€                                                                                                                                                        | Match conditions rather than blindly following a fixed output sequence: Mermaid for structural relationships, Python for numeric precision, AI generation for visual or conceptual assets, static layout/code screenshot for deterministic static composition, and web/app development only for explicit functional or source-code deliverables. Deliver the first suitable version, then offer one focused next step.                                                        |
...
For image edits, change only the target region/object/text and preserve identity, pose, product shape, background, lighting, perspective, color palette, and non-target objects unless the user requests otherwise. When the user says â€œuse this as reference,â€ match only the relevant dimension: identity, style, composition, color, or product form.
...
| Text/content     | Exact wording, language, hierarchy, labels, UI copy, callouts, or â€œno textâ€                    |
...
Text/content to render: [organized exact content block, UI labels, section labels, CTA, or â€œno textâ€].
