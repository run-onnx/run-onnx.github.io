/**
 * Tensor UI â€” Mineralische Topologie
 * Helles wissenschaftliches Atelier; asymmetrischer Instrumentenfluss, Mineral Teal als Statusfarbe,
 * DM Mono fÃ¼r Werte und Manrope fÃ¼r erklÃ¤rende Sprache. Keine Cyberpunk-Ã„sthetik, ein Fokus pro Modul.
 */
...
type MiniCategory = "EntwÃ¼rfe" | "Lernen" | "Experimente" | "Sammlung";
type MiniModel = {
  id: string;
  name: string;
  createdAt: number;
...
const MAX_CODEX_BYTES = 250 * 1024;
const MAX_MATRIX_CELLS = 256;
const MINI_MODEL_STORAGE_KEY = "tensor-mini-models-v1";
...
const MATERIALS: Array<{
...
}> = [
  {
    ...
    label: "Float",
    meta: "Plasma / Flux",
    description: "Weiche, kontinuierliche Energie fÃ¼r lernende Strukturen.",
  },
  {
    id: "int32",
    label: "Integer",
    meta: "Voxel / Cores",
    description: "Kristalline, gezÃ¤hlte Einheiten mit klarer Kante.",
  },
  {
    id: "bool",
    label: "Boolean",
    meta: "Trigger / Pulse",
    description: "Ein prÃ¤ziser Wechsel zwischen an und aus.",
  },
];
...
const SHAPE_PRESETS = [
  ...
  { label: "2D", value: "16 Ã— 16", note: "Grid" },
  { label: "3D", value: "6 Ã— 6 Ã— 6", note: "Field" },
];
...
const FIRST_MODEL_STEPS = [
  {
    title: "Eine Matrix fÃ¼hlen",
    copy: "Beginne mit einem kleinen 4Ã—4-Feld. Die Form wird sofort in der Vorschau spÃ¼rbar.",
    action: "4Ã—4 aufbauen",
  },
  {
    title: "Ein Muster schreiben",
    copy: "Gib deiner Matrix ein klares Muster. Jede Zelle zÃ¤hlt, jede Farbe antwortet.",
...
function parseShape(raw: string): number[] | null {
  ...
  if (!compact) return [];
  const parts = compact.split(/[Ã—x,\s]+/).filter(Boolean);
...
function readMiniModels(): MiniModel[] {
  try {
    const stored = window.localStorage.getItem(MINI_MODEL_STORAGE_KEY);
...
function parseTensorValues(raw: string, dtype: DType) {
  ...
  for (const token of tokens) {
    if (dtype === "bool") {
      ...
      else return { values, error: `â€ž${token}â€œ ist kein Boolean.` };
      continue;
    }
    const value = Number(token.replace(",", "."));
    if (!Number.isFinite(value))
      return { values, error: `â€ž${token}â€œ ist keine Zahl.` };
    if (dtype === "int32" && !Number.isInteger(value))
      return { values, error: `â€ž${token}â€œ ist keine ganze Zahl.` };
    values.push(value);
  }
  return { values, error: "" };
}
...
function decodeVarint(data: Uint8Array, start: number) {
  ...
  while (offset < data.length) {
    ...
    if (multiplier > Number.MAX_SAFE_INTEGER)
      throw new Error("Die ONNX-Datei enthÃ¤lt eine zu groÃŸe Zahl.");
  }
...
function decodeProto(data: Uint8Array) {
  ...
  while (offset < data.length) {
    ...
    if (!field)
      throw new Error("Die ONNX-Datei enthÃ¤lt ein ungÃ¼ltiges Feld.");
    if (wire === 0) {
      ...
      if (offset + length > data.length)
        throw new Error("Die ONNX-Datei ist unvollstÃ¤ndig.");
      ...
      throw new Error(
        "Diese ONNX-Datei verwendet ein nicht unterstÃ¼tztes BinÃ¤rfeld."
      );
    }
  }
  return fields;
...
export function inspectOnnx(data: Uint8Array, fileName: string): ImportedModel {
  ...
  if (!graphData)
    throw new Error("Diese Datei enthÃ¤lt keinen lesbaren ONNX-Graphen.");
...
export function buildOnnx(options: {
  ...
  const metadata = [
    ...
    ["tensor.dtype", dtype],
    ["tensor.shape", dims.length ? dims.join("Ã—") : "scalar"],
    ["tensor.operation", op],
...
export default function Home() {
  ...
  const [miniModelCategory, setMiniModelCategory] =
    useState<MiniCategory>("EntwÃ¼rfe");
  ...
  const routeNodes = useMemo(() => {
    ...
    return [
      ...
      ...operations,
      {
        id: "tensor-output",
        title: "Output",
        detail: `${shapeDescription} Â· ready`,
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem(
        MINI_MODEL_STORAGE_KEY,
        JSON.stringify(miniModels)
      );
    } catch {
      // Der lokale Speicher ist eine Komfortfunktion und darf den Studiofluss nie blockieren.
    }
  }, [miniModels]);
    ...
    const render = (time: number) => {
      ...
      for (const chain of chains) {
        for (let index = 1; index < chain.length - 1; index += 1) {
          ...
          const velocityY = (point.y - point.py) * 0.968;
          point.px = point.x;
          point.py = point.y;
          ...
          point.y += velocityY + (0.22 + force.y * 0.22 + ambient) * delta;
        }
      ...
      if (dtype === "float32") {
        ...
        for (let path = 0; path < 20; path += 1) {
          ...
          for (let step = 0; step < 62; step += 1) {
            ...
            const y = centerY + Math.sin(angle + p * 1.6) * radius * 0.56;
        ...
        for (let y = -4; y <= 4; y += 1) {
          for (let x = -4; x <= 4; x += 1) {
            ...
            const py = centerY + (x + y) * cell * 0.48 - depth * 12;
            context.save();
        ...
        for (let index = 0; index < 12; index += 1) {
          ...
          const y = centerY + Math.sin(angle) * scale * 0.42;
          context.beginPath();
  ...
  async function playSnap(frequency = 168) {
    ...
    try {
      ...
      if (context.audioWorklet && !workletLoaded.current) {
        const source = `
          class TensorPulse extends AudioWorkletProcessor {
            constructor(){ super(); this.phase = 0; this.remaining = 0; this.frequency = 180; this.port.onmessage = e => { this.frequency = e.data.frequency; this.remaining = 2400; }; }
            process(inputs, outputs){ const out = outputs[0][0]; for(let i=0;i<out.length;i++){ const env = this.remaining > 0 ? this.remaining / 2400 : 0; out[i] = Math.sin(this.phase) * env * .075; this.phase += 2 * Math.PI * this.frequency / sampleRate; this.remaining--; } return true; }
          } registerProcessor('tensor-pulse', TensorPulse);`;
      ...
      if (workletLoaded.current) {
        const node = new AudioWorkletNode(context, "tensor-pulse");
        node.connect(context.destination);
        node.port.postMessage({ frequency });
        window.setTimeout(() => node.disconnect(), 140);
        return;
      }
      ...
      // Audio ist eine haptische Zusatzebene und darf den eigentlichen Flow nie unterbrechen.
    }
  }
  ...
  function addCodexItem(name: string, type: string, content: string) {
    ...
    if (!content.trim() || size > MAX_CODEX_BYTES) {
      showCrow();
      return;
    }
  ...
  async function consumeFiles(files: FileList | File[]) {
    for (const file of Array.from(files)) {
      ...
      if (!allowed || file.size > MAX_CODEX_BYTES) {
        showCrow();
        continue;
      }
  ...
  async function importOnnx(file?: File) {
    ...
    if (file.size > 8 * 1024 * 1024) {
      setImportError(
        "FÃ¼r den lokalen Inspektor sind Dateien bis 8 MB gedacht."
      );
      showCrow();
      return;
    }
    try {
      ...
      if (importedDtype) {
        ...
        setShapeText(importedDims.join(" Ã— "));
  ...
  function exportRouteCard() {
    ...
    const nodes = cardNodes
      .map((node, index) => {
        ...
        const nextY = 270 + ((index + 1) % 2 ? -32 : 32);
        const rope = hasNext
          ? `<path d="M ${x + 43} ${y} C ${x + 72} ${y}, ${nextX - 72} ${nextY}, ${nextX - 43} ${nextY}" fill="none" stroke="#0b7c78" stroke-width="4" stroke-linecap="round"/><path d="M ${x + 43} ${y - 1} C ${x + 72} ${y - 1}, ${nextX - 72} ${nextY - 1}, ${nextX - 43} ${nextY - 1}" fill="none" stroke="#eafff7" stroke-opacity=".65" stroke-width="1"/>`
          : "";
  ...
  function saveMiniModel() {
    if (!valid) return;
    const name = miniModelName.trim() || `${typeLabel(rank)} Â· ${operation}`;
  ...
  function composeMiniModels() {
    if (!composePartner) {
      setComposeNotice(
        "WÃ¤hle zuerst ein gespeichertes Modell als zweiten Strang."
      );
      return;
    }
    if (
      ...
      setComposeNotice(
        "FÃ¼r Compose brauchen beide Modelle dieselbe Form und denselben Materialtyp."
      );
      showCrow();
      return;
    }
    ...
    setComposeNotice(
      `Verbunden: Aktueller Tensor + ${composePartner.name}. Die Werte werden bei gleicher direkter Form gemittelt, Codex-Fragmente zusammengefÃ¼hrt.`
    );
  ...
  return (
    ...
    >
      ...
      <header className="topbar">
        <div className="brand-lockup" aria-label="Tensor Studio">
          <img className="brand-mark" src={MARK_ASSET} alt="" />
          <span className="brand-name">tensor</span>
        ...
        <div className="top-actions">
          <button
            ...
            title="GefÃ¼hrtes erstes Modell Ã¶ffnen"
          >
            <Sparkles size={16} />
          ...
          <button
            ...
            title="ONNX-Datei lokal Ã¶ffnen"
          >
            <FolderOpen size={16} />
            <span>ONNX Ã¶ffnen</span>
          </button>
          <input
            ref={modelInputRef}
            type="file"
            ...
            <div className="module-body">
              {activeModule === 0 && (
                <section
                  ...
                  aria-label="Datentyp auswÃ¤hlen"
                >
                  <p className="lead">
                ...
                >
                  ...
                  <div className="shape-input-wrap">
                    ...
                    >
                      ...
                      {valid ? "Valide" : "max. 6 Dimensionen / 16.384 Werte"}
                    </span>
                  </div>
              ...
              {activeModule === 2 && (
                <section
                  className="content-panel"
                  aria-label="Tensoroperation und Werte wÃ¤hlen"
                >
                  <p className="lead">
                  ...
                  <div className="seed-row">
                    ...
                    >
                      Neu wÃ¼rfeln <MoveRight size={14} />
                    </button>
                  </div>
                  <section
                      ...
                      <>
                        {canUseMatrixGrid && (
                          <div
                            ...
                            role="group"
                            aria-label="Eingabeform wÃ¤hlen"
                          >
                            <button
                              className={
                        ...
                        {valueEditor === "grid" && canUseMatrixGrid ? (
                          <div className="matrix-grid-wrap">
                            <div className="matrix-grid-label">
                              <span>
                                {matrixRows} Zeilen Ã— {matrixCols} Spalten
                              </span>
                        ...
                        {!canUseMatrixGrid && rank === 2 && (
                          <p className="grid-note">
                            FÃ¼r das Grid sind bis zu {MAX_MATRIX_CELLS}{" "}
                ...
                >
                  ...
                  >
                    ...
                    <div>
                      ...
                      <span>.txt, .json oder .svg Â· bis 250 KB</span>
                    </div>
              ...
              {activeModule === 4 && (
                ...
                >
                  <p className="lead">
                    ...
                    wandert â€” als fÃ¼hlbare Spur statt als Fachchinesisch.
                  </p>
                  <div
                  ...
                  <div className="route-search">
                    <label htmlFor="route-search">Spur lesen</label>
                    <input
                      id="route-search"
                      value={graphQuery}
                  ...
                  >
                    <div className="scene-heading">
                      <div>
                        <span className="option-meta">Tensor Szene</span>
                        <strong>
                          Zwei StrÃ¤nge kÃ¶nnen eine neue, ruhige Einheit
                          bilden.
                        </strong>
                      </div>
                    ...
                    <div className="scene-stage">
                      {sceneModels.map((model, index) => (
                        ...
                        >
                          ...
                          <small>
                            {model.shapeText || "Scalar"} Â· {model.operation}
                          </small>
                        </button>
                      ))}
                      ...
                      <div>
                        ...
                        <strong>
                          {composePartner
                            ...
                            : "WÃ¤hle einen zweiten Strang in der Szene."}
                        </strong>
                      </div>
                      <button
                    ...
                    >
                      ...
                      <span>
                        ...
                        <small>
                          {shelfOpen
                            ? "Speicher schlieÃŸen"
                            : "Speichern, laden oder aufrÃ¤umen"}
                        </small>
                      </span>
                      {shelfOpen ? (
                        <ChevronUp size={17} />
                      ...
                      <div className="shelf-details">
                        <div className="save-model-row">
                          <input
                            ...
                            maxLength={42}
                            placeholder="Name fÃ¼r dieses Modell â€¦"
                          />
                          <select
                            ...
                            aria-label="Kategorie fÃ¼r neues Mini-Modell"
                          >
                        ...
                        {visibleMiniModels.length ? (
                          ...
                          <p className="shelf-empty">
                            ...
                            Speichere ein kleines Modell, um es spÃ¤ter wieder
                            hervorzuholen.
                          </p>
                        )}
                      </div>
                  ...
                  <p className="graph-note">
                    Die Sensorfreigabe ist optional und bleibt nur fÃ¼r diesen
                    Moment aktiv. Ohne sie reagiert die Ansicht auf BerÃ¼hrung
                    und Maus.
                  </p>
                </section>
              )}
          ...
          >
                  ...
                  <span>
                    ...
                    <small>
                      {importedModel.supported
                        ? "Im Studio Ã¼bernommen"
                        : "Lesbar, aber nicht vollstÃ¤ndig editierbar"}
                    </small>
                  </span>
                  {inspectorOpen ? (
                    <ChevronUp size={17} />
                  ...
                  <div className="inspector-details">
                    <dl className="import-facts">
                      ...
                      <div>
                        <dt>Format</dt>
                        <dd>
                          IR {importedModel.irVersion} Â· {importedModel.opset}
                        </dd>
                      </div>
                    </dl>
                    <div className="inspector-section">
                      ...
                      <div className="node-chips">
                        {importedModel.nodeOps.length ? (
                          ...
                          <span>Keine Nodes gefunden</span>
                        )}
                      </div>
                    </div>
                    <div className="inspector-section">
                      ...
                      {importedModel.metadata.length ? (
                        ...
                        <p className="inspector-empty">
                          Dieses Modell trÃ¤gt keine zusÃ¤tzlichen Metadaten.
                        </p>
                      )}
                    </div>
                  </div>
            ...
            {!valid && (
              <div className="validation-callout">
                <ShieldCheck size={16} />
                {modelValid
                  ? "Die Zahl deiner echten Werte muss zur Form passen."
                  : "Eine gÃ¼ltige Form lÃ¤sst den lokalen Export erscheinen."}
              </div>
            )}
          </aside>
        </main>
      ...
      {primerOpen && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Erstes Modell"
        >
          <section className="primer-card">
            <button
              className="primer-close"
              onClick={() => setPrimerOpen(false)}
              aria-label="Erstes Modell schlieÃŸen"
            >
              <X size={17} />
            </button>
            <span className="option-meta">
              Erstes Modell Â· {primerIndex + 1} / 3
            </span>
